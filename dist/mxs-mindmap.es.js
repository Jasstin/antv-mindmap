...
const edit = (id) => {
  ...
  const { name, type, fontSize, width, height } = (_b = NodeData2._cfg) == null ? void 0 : _b.model;
  ...
  EditInput$1.handleInputBlur = (name2) => {
    if (name2.trim().length) {
      emitter.emit("onAfterEdit", name2.replace(/\s/g, ""));
      update(id, name2.replace(/\s/g, ""));
    } else if (name2 === "") {
      deleteOneNode(id);
    }
    Tree2.off("wheelzoom");
  ...
  Tree2.on("wheelzoom", () => {
    ...
    EditInput$1.showInput(x2, y2, width * ratio, height * ratio, name, fontSize * ratio, type, radius * ratio, ratio);
  });
};
...
const _hoisted_2 = /* @__PURE__ */ createElementVNode("textarea", { id: "node-input" }, null, -1);
const _hoisted_3 = [
  _hoisted_1,
  _hoisted_2
];
